...
import { renderCodexSkills } from "./skills-render.ts";
...
export function defaultTemplatePaths(repoRoot: string): string[] {
  return [
    ...
    // The Codex trace skill is host-specific; its canonical source is the
    // override beside the Claude trace skill. The codex/skills/ tree itself is
    // generated by renderCodexSkills, not hand-stamped.
...
export function runRelease(options: {
  ...
  // Regenerate the Codex skills tree from the freshly stamped Claude skills so
  // both agents ship the full skill set at the same pinned version.
  const renderedCodexSkills = renderCodexSkills(options.repoRoot);
  ...
  verifyPinnedTemplates({
    expectedVersion: options.nextVersion,
    templatePaths: renderedCodexSkills,
  });
}
