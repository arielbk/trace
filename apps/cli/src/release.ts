...
import { findArtifactDrift, renderArtifacts } from "./skills-render.ts";
...
export function stampReleaseVersion(
  ...
  const templatePaths = options.templatePaths ?? [];
  const versionedManifestPaths = options.versionedManifestPaths ?? [];
...
/**
 * Stamp `apps/cli/package.json` to `nextVersion`, then delegate all artifact
 * production and verification to the render module. Throws if any artifact
 * remains stale after rendering (unstamped pin, wrong manifest version, etc.).
 */
export function prepareReleaseArtifacts(options: {
  repoRoot: string;
  nextVersion: string;
}): void {
  const { repoRoot, nextVersion } = options;
  ...
  renderArtifacts(repoRoot, nextVersion);

  const drift = findArtifactDrift(repoRoot, nextVersion);
  if (drift.length > 0) {
    throw new Error(
      `Release verification failed — unstamped or stale artifacts:\n${drift.join("\n")}`,
    );
  }
}
