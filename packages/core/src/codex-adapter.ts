...
export function parseCodexTranscript(input: CodexTranscriptInput): ParsedCodexSession {
...
export function scanCodexSessions(codexHome: string): ParsedCodexSession[] {
  ...
  return transcriptPaths.map((entry) =>
    parseCodexTranscriptFile(entry.transcriptPath, { expectedThreadId: entry.expectedThreadId }),
  );
}

function readCodexSessionIndex(codexHome: string): Array<{ transcriptPath: string; expectedThreadId?: string }> {
...
function findJsonlFiles(directoryPath: string): Array<{ transcriptPath: string; expectedThreadId?: string }> {
  if (!existsSync(directoryPath)) {
    return [];
  }

  return readdirSync(directoryPath, { withFileTypes: true })
    .flatMap((entry: Dirent): Array<{ transcriptPath: string; expectedThreadId?: string }> => {
...
function addUsage(tokenTotals: CodexTokenTotals, usage: CodexUsage | undefined): void {
  if (!usage) {
    return;
  }
