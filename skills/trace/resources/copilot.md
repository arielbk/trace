...
and submits a nudge to consult EQNX. Follow that nudge when the session is not
...
home). Run EQNX commands from inside the Copilot session so that lock can be
